...
export default function ActiveSessionPage({ session, onSessionEnd, onSettings }) {
  ...
  const allowedSites = [
    ...(session.whitelist || []),
    ...(session.allowedSites || []).map((s) => s.url),
  ].filter(Boolean);
        ...
        <div className="flex items-center gap-2.5">
          <div className="relative flex items-center justify-center">
            <div
              ...
              style={{ background: 'rgba(185, 28, 28, 0.12)', animation: 'pulseRing 2s ease-out infinite' }}
            />
            <div
        ...
        {allowedSites.length > 0 && (
          <div className="bg-black/30 rounded-xl px-4 py-3 border border-red-500/30">
            <div className="flex items-center gap-2 mb-2.5">
              ...
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-widest">Allowed Sites</p>
            </div>
            <div className="space-y-1.5">
              {allowedSites.map((site, i) => (
                <div key={i} className="flex items-center gap-2">
                  <div className="w-1.5 h-1.5 rounded-full bg-green-600 shrink-0" />
                  <p className="text-xs text-gray-300 truncate">{site}</p>
                </div>
              ))}
            </div>
          </div>
