...
function matchUrl(url, pattern) {
  ...
  try {
    ...
    // In such cases, it's safer to assume no match to maintain blocking integrity.
...
chrome.alarms.onAlarm.addListener(async (alarm) => {
  ...
  if (alarm.name === 'warden-break-end') {
    ...
    session.breaks.nextBreak = Date.now() + session.breaks.interval * 60 * 1000; // Corrected from Date.Now()
    await setActiveSession(session);
